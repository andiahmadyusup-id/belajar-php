<?php
$nama = "Yos hi";
echo $nama . "<br>";

$nama_array = array("Dava", "Tegep", "Ferdinand", "Satrio", "Andi");
print_r($nama_array); echo "<br>";

echo $nama_array[0] . "<br>";
echo $nama_array[1] . "<br>";
echo $nama_array[2] . "<br>";
echo "<hr>";

// Versi Foreach
foreach ($nama_array as $namaLengkap){
    echo $namaLengkap . "<br>";
}
echo "<hr>";

// Versi For
for ( $i = 0; $i < COUNT ($nama_array); $i++){
    echo $nama_array[$i] . "<br>";
}

//Multiple Array
$kelas11rpl2 = array(
    array("Satrio", "Subang", "Mancing"),
    array("Tegep", "Bandung", "Membaca Buku"),
    array("Dian", "Jakarta", "Main Game", "Menari")
);

echo "<pre>";
print_r($kelas11rpl2);
echo "</pre";
echo "<br>";
echo $kelas11rpl2[0][0] . "<br>"; //Satrio
echo $kelas11rpl2[1][0] . "<br>"; //Tegep
echo $kelas11rpl2[0][2] . "<br>"; //Mancing
echo $kelas11rpl2[2][1] . "<br>"; //Jakarta
echo $kelas11rpl2[1][2] . "<br>"; //Membaca Buku
echo $kelas11rpl2[2][3] . "<br>"; //Menari


foreach($kelas11rpl2 as $kelas1){
    foreach($kelas1 as $kelas){
        echo $kelas . "<br>";
    }
}


